...
# ======
# inputs:
# - 2 ints (we want to play the verse for the 1st int and all subsequent
#   verses upto and including the 2nd int)
# outputs:
# - str (one or more verses of the song, separated by "\n")
# reqs:
# - output a str consisting of the specified verses of a song
# rules:
# - none
# struct:
# - str
# algo:
# - init a str to ''
# - for start_bottle downto end_bottle
#   - add the current verse to the str
#   - unless it's the last line, add a newline separator
# - rtn the str
...
class BeerSong
  ...
  def verses(start_bottle, end_bottle)
    song = ''
    start_bottle.downto(end_bottle) do |bottle|
      song << verse(bottle)
      song << "\n" unless bottle == end_bottle
    end
    song
  end

  # #verse() is a little repetitive, but much simpler than defining the logic to
  # handle all the possible variations of the dif verse strs
  def verse(bottle)
    case bottle
    when 3..99
